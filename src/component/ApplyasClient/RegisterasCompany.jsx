...
const CompanyForm = () => {
  const [formData, setFormData] = useState({
    ...
    purposeOfJoining: "",
    contactPersonName: "",
    companyLevel: "startup", // Default value
  });
  ...
  const validateForm = () => {
    ...
    // Company Level validation (optional)
    if (formData.companyLevel === "") {
      errors.companyLevel = "Please select a company level.";
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:3000/api/company/submit",
        formData
      );
  ...
  return (
    <div className="company-form-container">
      {submitted ? (
        <div className="thank-you-message-company">
          <h1>Thank You!</h1>
        ...
        <>
          <h1 className="company-form-title">Company Registration Form</h1>
          <form className="company-registration-form" onSubmit={handleSubmit}>
            ...
            <div className="form-group">
              ...
              {errorMessages.companyLevel && (
                <p className="error-message">{errorMessages.companyLevel}</p>
              )}
            </div>
